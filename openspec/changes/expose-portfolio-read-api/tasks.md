...
- [ ] 1.1 Mover a valorização por posição (`_valorizar`, `_preco_opcao`) e a
      montagem do resumo (`_resumo_carteira`) de `src/report/daily.py` para
      uma função pública em `src/market/valuation.py` (`design.md`,
      decisão 1).
- [ ] 1.2 Devolver a visão como estrutura tipada (dataclass), não dict solto,
      já que agora ela atravessa duas camadas — relatório e API.
- [ ] 1.3 Fazer `src/report/daily.py` consumir a função extraída, mantendo
      apenas a renderização Markdown.
- [ ] 1.4 Confirmar que `tests/test_report_daily.py` passa **sem alteração de
...
- [ ] 1.5 Testes da função extraída em `tests/test_market_valuation.py`:
...
- [ ] 3.6 Carteira vazia e ausência de sugestões respondem com sucesso
      representando o vazio, nunca com erro.
- [ ] 3.7 Testes com `TestClient`: desfecho com motivos e sem execução
...
- [ ] 3.8 Teste de que os números da API coincidem com os do relatório para o
      mesmo estado — a prova de que vieram da mesma função de domínio (spec,
      cenário "Valores coincidem com os do relatório").

## 4. Contrato OpenAPI

- [ ] 4.1 Conferir que o schema publicado cobre os três endpoints e que os
      ...
      tipado").
- [ ] 4.2 Adicionar um comando para salvar o schema em arquivo, para que a
      geração de tipos não dependa da API estar no ar.

## 5. Tipos no `opcoes-ia-web`

- [ ] 5.1 Adicionar `openapi-typescript` como dependência de
      desenvolvimento e um script npm de geração.
- [ ] 5.2 Gerar os tipos a partir do schema e versionar o arquivo gerado, com
      comentário de que é gerado e não deve ser editado à mão.
- [ ] 5.3 Escrever um cliente mínimo tipado que consuma o endpoint de
      carteira, provando o contrato de ponta a ponta.
- [ ] 5.4 Confirmar que `npm run build` (que roda `tsc -b`) aceita o
      resultado.

## 6. Documentação

- [ ] 6.1 `CLAUDE.md`: como subir a API, onde ela mora, e os três limites
      (não decide, não dispara, não escreve).
- [ ] 6.2 `CLAUDE.md`: registrar que a interface mostra o desfecho da
      ÚLTIMA execução, não do instante — se a avaliação não rodou hoje, o
      que aparece é de outro dia, e a data precisa estar visível.
- [ ] 6.3 `docs/ARQUITETURA.md`: a Fase 4 deixa de ser apenas relatório
      estático; registrar a separação em dois repositórios e por quê.
- [ ] 6.4 `README.md` do `opcoes-ia-web`: substituir o placeholder do comando
      da API pelo comando real e documentar a geração de tipos.

## 7. Validação de ponta a ponta

- [ ] 7.1 Rodar `pytest` completo apontando para o banco descartável.
- [ ] 7.2 Com a API no ar contra o Neon, conferir que o endpoint de carteira
      devolve o mesmo patrimônio que o relatório gerado no mesmo momento.
- [ ] 7.3 Subir o dev server do Vite e confirmar que o cliente tipado busca a
      carteira sem erro de CORS.
- [ ] 7.4 Confirmar que a API responde apenas em `127.0.0.1` — uma requisição
